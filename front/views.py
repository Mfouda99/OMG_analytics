...
import json
from expa_data.models import ExpaApplication, SignupPerson, PodioSignupOGV, PodioSignupOGTa, PodioSignupOGTe
...
def get_podio_signups_count(programme_name, start_date, end_date, lc_filter=None):
    """
    Get signup count from appropriate Podio model based on programme
    """
    if programme_name == 'OGV':
        queryset = PodioSignupOGV.objects.filter(
            created_at__gte=start_date,
            created_at__lte=end_date
        )
    elif programme_name == 'OGTa':
        queryset = PodioSignupOGTa.objects.filter(
            created_at__gte=start_date,
            created_at__lte=end_date
        )
    elif programme_name == 'OGTe':
        queryset = PodioSignupOGTe.objects.filter(
            created_at__gte=start_date,
            created_at__lte=end_date
        )
    else:
    ...
    # Apply LC filter if provided - handle naming differences between EXPA and Podio
    if lc_filter:
        if lc_filter == 'Piraeus (UniPi)':
            # UniPi appears as both "Patras" and "UniPi" in Podio data
            queryset = queryset.filter(Q(home_lc__icontains='Patras') | Q(home_lc__icontains='UniPi'))
        elif lc_filter == 'UoI (EXP)':
            # UoI (EXP) appears as "UoI" in Podio data
            queryset = queryset.filter(home_lc__icontains='UoI')
        elif lc_filter == 'Volos (EXP)':
            # Volos (EXP) appears as "Volos" in Podio data  
            queryset = queryset.filter(home_lc__icontains='Volos')
        elif lc_filter == 'Athens':
            # Athens appears as "ATHENS" in Podio data
            queryset = queryset.filter(home_lc__icontains='ATHENS')
        else:
            queryset = queryset.filter(home_lc__icontains=lc_filter)
...
def get_mc_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date)
...
def get_lc_ranking_data(start_date, end_date):
    ...
    lcs = [
        ...
        {'name': 'UNIPI', 'filter': 'Piraeus (UniPi)'},
        {'name': 'LC THESSALONIKI', 'filter': 'UoM THESSALONIKI'},
        {'name': 'IE UOI', 'filter': 'UoI (EXP)'},
...
def get_lc_athens_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'Athens')
...
def get_lc_auth_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'AUTH')
...
def get_lc_unipi_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'Piraeus (UniPi)')
...
def get_LC_UoM_Thessaloniki_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'UoM THESSALONIKI')
...
def get_IE_UOI_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'UoI (EXP)')
...
def get_IE_Volos_analytics_data(start_date, end_date):
    ...
    for programme_name, programme_ids in ogx_programmes.items():
        ...
        # Get signups for this programme in date range from Podio
        signups = get_podio_signups_count(programme_name, start_date, end_date, 'Volos (EXP)')
